...
**Comment posting, reporting, product submission, and waitlist signup use the DB counter, not the
binding.** The Workers `ratelimit` binding is what those endpoints should eventually use, and the
table above named it first. Nothing is deployed to Workers yet, so there is no binding to call and
the real choice is the counted layer or no limit at all. Counted is the stricter of the two, the
